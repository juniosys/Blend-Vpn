...
package atl.juniosys.blendvpn.tunnel.vpn;
			...
			boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering) throws java.lang.Exception
	{
		...
		try
		{
			...
			if ((Build.VERSION.SDK_INT == Build.VERSION_CODES.KITKAT && !release.startsWith("4.4.3")
                ...
                && mMtu < 1280) {
				BlendStatus.logInfo(String.format(Locale.US, "Forcing MTU to 1280 instead of %d to workaround Android Bug #70916", mMtu));
				mMtu = 1280;
			}
			builder.setMtu(mMtu);
			...
			for (NetworkSpace.IpAddress route : mRoutes.getPositiveIPList()) {
				try {
					if (multicastRange.containsNet(route))
						BlendStatus.logDebug("VPN: Ignoring multicast route: " + route.toString());
					else
			...
			tunFd = builder
					...
					.setConfigureIntent(BlendService.getGraphPendingIntent(mHostService.getContext()))
					.establish();
