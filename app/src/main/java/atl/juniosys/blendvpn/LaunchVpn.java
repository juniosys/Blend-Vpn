...
import atl.juniosys.blendvpn.tunnel.TunnelManagerHelper;
import atl.juniosys.blendvpn.tunnel.TunnelUtils;

public class LaunchVpn extends AppCompatActivity
	implements DialogInterface.OnCancelListener
{
	public static final String EXTRA_HIDELOG = "atl.juniosys.blendvpn.showNoLogWindow";
	...
	private void askForPW(final int type) {
        ...
        dialog.setNegativeButton(android.R.string.cancel,
			new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					BlendStatus.updateStateString("USER_VPN_PASSWORD_CANCELLED", "", R.string.state_user_vpn_password_cancelled,
	...
	private void showLogWindow() {
        Intent updateView = new Intent("atl.juniosys.blendvpn:openLogs");
        ...
        if (requestCode == START_VPN_PROFILE) {
            if (resultCode == Activity.RESULT_OK) {
				...
				if (!TunnelUtils.isNetworkOnline(this)) {
                    ...
                    BlendStatus.updateStateString("USER_VPN_PASSWORD", "", R.string.state_user_vpn_password,
                    ...
                    TunnelManagerHelper.startBlend(this);
                ...
                BlendStatus.updateStateString("USER_VPN_PERMISSION_CANCELLED", "", R.string.state_user_vpn_permission_cancelled,
					ConnectionStatus.LEVEL_NOTCONNECTED);

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
                    BlendStatus.logError(R.string.nought_alwayson_warning);
	...
	private void launchVPN() {
        ...
        if (intent != null) {
            BlendStatus.updateStateString("USER_VPN_PERMISSION", "", R.string.state_user_vpn_permission,
				ConnectionStatus.LEVEL_WAITING_FOR_USER_INPUT);
            // Start the query
            try {
                ...
                BlendStatus.logError(R.string.no_vpn_support_image);
                showLogWindow();
            }
        } else {
            onActivityResult(START_VPN_PROFILE, Activity.RESULT_OK, null);
