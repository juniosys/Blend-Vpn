...
import com.juniocam.blendvpn.logger.*;
...
public class SSLPayTunnel implements ProxyData
{
    ...
    public void close()
    {
        ...
        public void handshakeCompleted(HandshakeCompletedEvent handshakeCompletedEvent) {
            ...
            BlendStatus.logInfo(new StringBuffer().append("SSL: Supported protocols: <br>").append(Arrays.toString(val$sslSocket.getSupportedProtocols())).toString().replace("[", "").replace("]", "").replace(",", "<br>"));
            BlendStatus.logInfo(new StringBuffer().append("SSL: Enabled protocols: <br>").append(Arrays.toString(val$sslSocket.getEnabledProtocols())).toString().replace("[", "").replace("]", "").replace(",", "<br>"));
            BlendStatus.logInfo("SSL: Using cipher " + handshakeCompletedEvent.getSession().getCipherSuite());
            BlendStatus.logInfo("SSL: Using protocol " + handshakeCompletedEvent.getSession().getProtocol());
            BlendStatus.logInfo("SSL: Handshake finished");
        }
    }
    ...
    public SSLPayTunnel(String server, int port, String hostSni, String requestPayload) {
  ...
  this.requestPayload = requestPayload;
    ...
    public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException
    {
        ...
        String requestPayload = getRequestPayload(hostname, port);
            ...
            BlendStatus.logInfo("<strong>" + httpReponseFirstLine + "</strong>");
            ...
            String str2 = httpReponseFirstLine;
                    ...
                    if (parseInt == 200) {
                        ...
                        BlendStatus.logInfo("<b>HTTP/1.1 200 Connection established</b>");
                        return this.mSocket;
                    }
        ...
        try {
            ...
            if (sSLContext.getSocketFactory() instanceof android.net.SSLCertificateSocketFactory && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR1) {
                ...
                try {
                    ...
                    BlendStatus.logInfo("Setting up SNI...");
                } catch (Throwable e) {
            ...
            BlendStatus.logInfo("Starting SSL Handshake...");
