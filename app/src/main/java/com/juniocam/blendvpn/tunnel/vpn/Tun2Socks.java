package com.juniocam.blendvpn.tunnel.vpn;
...
import com.juniocam.blendvpn.util.StreamGobbler;
import android.support.v4.content.ContextCompat;
import com.juniocam.blendvpn.logger.BlendStatus;
import com.juniocam.blendvpn.util.CustomNativeLoader;
...
public class Tun2Socks extends Thread implements StreamGobbler.OnLineListener {
	...
	public void run() {
		...
		if (mListener != null) {
 		...
 		try {
			...
			BlendStatus.logException("Tun2Socks Error", e);
		} catch (Exception e) {
			BlendStatus.logDebug("Tun2Socks Error: " + e.getMessage());
		}
	...
	*/
	...
	public void onLine(String log){
		BlendStatus.logDebug("Tun2Socks: " + log);
	}
	...
	private boolean sendFd(ParcelFileDescriptor fileDescriptor, File toFile) throws InterruptedException {
		...
		BlendStatus.logDebug("Enviando Fd para sock");
